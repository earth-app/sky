...
- `classifyAvailability()` turns that into a verdict deterministically. **A failed `appTransaction` outranks everything below it**: Apple documents `AppTransaction.shared` as throwing when the transaction is unavailable _or_ the account is not authenticated with the App Store, which is the same condition that empties the catalogue - StoreKit reached the store (a storefront resolved) but cannot tell which app this install is, so it drops every identifier. That is `no_app_identity`, and the fix is on the device (Sandbox Apple Account, or install from TestFlight), never in App Store Connect. Only once the app transaction succeeds does the **`all_products_missing` (bundle id / app record) vs `some_products_missing` (Missing Metadata)** split mean anything. `environment === 'Xcode'` means a local `.storekit` file is serving products and ASC was never consulted.
- The bundle id is read from **two** sources: `bundleId` (what the App Store thinks, via the app transaction) and `localBundleId` (what the binary says, via `App.getInfo().id`). The first dies with the app transaction; the second is what keeps a genuine mismatch visible in exactly that case.
